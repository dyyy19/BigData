...
A contribution-gated platform for archiving and accessing past NTU exams, homework, and project files. Built as the final project for Big Data Systems, Spring 2026, National Taiwan University.

**Live demo:** https://ntuarchive.vercel.app

## What this is

NTU students currently get past exams through personal networks — assigned mentors (直屬學長姐), classmates, or scattered forum posts. Students without those connections (freshmen, exchange students, anyone without the right social ties) are left out. NTU Archive replaces that informal, gatekept system with a shared, searchable archive open to any student with an `@ntu.edu.tw` email.

Access is structured as a contribution-gated freemium model: uploading a resource earns merit points, and previewing another student's upload costs merit points (paid to the uploader). This converts the existing "social cost" of asking around for materials into a transparent, platform-native exchange.

See the full project report (`B13902092.pdf`, also in this repository) for target customer analysis, demand evidence, and system design rationale.
...
- **One-time nickname** — set once at signup or via account settings, then locked
...
2. Open `index.html` with a local server (e.g. the VS Code "Live Server" extension, or any static file server — opening the file directly via `file://` may cause issues with browser security policies for some Supabase calls)
...
Note: the publishable key is safe to expose in client-side code by design — it only grants the access explicitly allowed by the project's Row Level Security policies. No secret/service-role key is present anywhere in this codebase.
